...
#%% 💾 Load Model and Metrics with Dynamic Paths
model_path = os.path.join(BASE_DIR, "models", "best_model.pkl")
metrics_path = os.path.join(BASE_DIR, "results", "metrics.json")
data_path = os.path.join(BASE_DIR, "data", "CO2_Emissions_cleaned.csv")
...
if df is not None:
    with st.expander(T["make_lookup"]):
        ...
        model_selected = st.selectbox("Model", sorted(df[df["Make"] == make]["Model"].dropna().unique()))
        match = df[(df["Make"] == make) & (df["Model"] == model_selected)]
        if not match.empty:
            avg = match["CO2 Emissions(g/km)"].mean()
            st.info(f"🔍 Avg CO₂ for **{make} {model_selected}**: {avg:.2f} g/km")
